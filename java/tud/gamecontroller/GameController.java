...
public class GameController<
	...
	private int maxStepNumber=Integer.MAX_VALUE;
	...
	public void runGame() throws InterruptedException {
		...
		gameStart();
		while(!currentState.isTerminal() && step<=maxStepNumber){
		...
		if(step>maxStepNumber) {
			String message = "match too long (>"+maxStepNumber+" steps)!";
			GameControllerErrorMessage errorMessage = new GameControllerErrorMessage(GameControllerErrorMessage.GAME_ERROR, message);
	...
	private JointMoveInterface<TermType> gamePlay(int step, JointMoveInterface<TermType> priorJointMove, State<TermType, ReasonerStateInfoType> priorState) throws InterruptedException {
		...
		for(RoleInterface<TermType> role:game.getOrderedRoles()){
			...
			MoveInterface<TermType> lastMoveByPlayer = null;
			if (priorJointMove != null) {
				lastMoveByPlayer = priorJointMove.get(role);
			}
			playerthreads.add(new PlayerThreadPlay<TermType, State<TermType, ReasonerStateInfoType>>(role, player, match, lastMoveByPlayer, seesTerms, playclock*1000+EXTRA_DEADLINE_TIME));
		}
	...
	private void gameStop(JointMoveInterface<TermType> priorJointMove, State<TermType, ReasonerStateInfoType> priorState) throws InterruptedException {
		...
		for(RoleInterface<TermType> role:game.getOrderedRoles()){
			...
			playerthreads.add(new PlayerThreadStop<TermType, State<TermType, ReasonerStateInfoType>>(role, player, match, lastMoveByPlayer, seesTerms, playclock*1000+EXTRA_DEADLINE_TIME));
		}
	...
	public void setMaxStepNumber(int maxStepNumber) {
		if(maxStepNumber>0) {
			...
			this.maxStepNumber=Integer.MAX_VALUE;
		}
	}
