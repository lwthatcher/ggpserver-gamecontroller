...
parse_gdl_term_list_string(String,List) :-
	parse_string(String, gdl_term_list(List)).
...
gdl_term_list(Terms) --> gdl_left_parens, gdl_white_star, gdl_term_star_right_parens(Terms).
...
gdl_integer_string_list_nonempty([C1,C2|List]) --> gdl_minus(C1), gdl_digit(C2), !, gdl_integer_string_list(List).
...
gdl_message2(start(MatchID, Role, Axioms, StartClock, PlayClock)) -->
	...
	gdl_left_parens, gdl_white_star, gdl_knowledge_base(Axioms), gdl_right_parens, gdl_white_star,
	gdl_integer(StartClock), gdl_white_star,
	gdl_integer(PlayClock).
gdl_message2(Msg) -->
	[112,108,97,121], !, % ="play"
	gdl_play_or_stop_message(play, Msg).
gdl_message2(Msg) -->
	[114,101,112,108,97,121], !,
	gdl_play_or_stop_message(replay, Msg).
gdl_message2(Msg) -->
	[115,116,111,112], !, % ="stop"
	gdl_play_or_stop_message(stop, Msg).

gdl_play_or_stop_message(PlayOrStop, Msg) -->
	gdl_white_plus,
	gdl_constant(MatchID),
	gdl_play_or_stop_message2(PlayOrStop, MatchID, Msg).

gdl_play_or_stop_message2(PlayOrStop, MatchID, Msg) --> % GDL-II
	gdl_white_plus,
	gdl_integer(Turn), !,
	gdl_white_plus,
	gdl_term(LastMove),
	gdl_white_star,
	gdl_term_list(Percepts),
	{ Msg=..[PlayOrStop, MatchID, Turn, LastMove, Percepts] }.
gdl_play_or_stop_message2(PlayOrStop, MatchID, Msg) --> % GDL-I
	gdl_white_star,
	gdl_term_list(Actions),
	{ Msg=..[PlayOrStop, MatchID, Actions] }.
...
translate_to_sexpr_string(V, S) :-
	open(string(""), write, Stream),
	write_to_stream_as_sexpr(Stream, V),
	get_stream_info(Stream, name, S),
	close(Stream).

:- mode write_to_stream_as_sexpr(++, ?).

% this first clause will probably not be used and is just here in case there is something wrong
write_to_stream_as_sexpr(Stream, V) :- var(V), !, printf(Stream, "?%w", [V]).

write_to_stream_as_sexpr(Stream, []) :- !, write(Stream, "nil").
write_to_stream_as_sexpr(Stream, [E|L]) :- !,
	write_to_stream_as_sexpr(Stream, E),
	write_to_stream_as_sexpr_list(Stream, L).

write_to_stream_as_sexpr(Stream, Head:-Tail) :- !,
	(Tail = true ->
		write_to_stream_as_sexpr(Stream, Head)
	;
		write(Stream, "(<="),
		my_and_to_list(Tail, TailList),
		write_to_stream_as_sexpr_list(Stream, [Head|TailList]),
		write(Stream, ")")
	).

write_to_stream_as_sexpr(Stream, T) :-
	T=..[F,A1|As],!, % not an atom
	write(Stream, "("),
	write_to_stream_as_sexpr(Stream, F),
	write_to_stream_as_sexpr_list(Stream, [A1|As]),
	write(Stream, ")").

write_to_stream_as_sexpr(Stream, T) :- write(Stream, T). % T must be some atomic term

:- mode write_to_stream_as_sexpr(++, +).
write_to_stream_as_sexpr_list(_Stream, []).
write_to_stream_as_sexpr_list(Stream, [A|As]) :-
	write(Stream, " "),
	write_to_stream_as_sexpr(Stream, A),
	write_to_stream_as_sexpr_list(Stream, As).
...
:- mode my_and_to_list(+,?).
my_and_to_list(And,L) :-
        my_and_to_list1(And,L,[]).

:- mode my_and_to_list1(+, ?, ?).
my_and_to_list1((P1, P2),Left,Right) :- !,
        my_and_to_list1(P1,Left,Middle),
        my_and_to_list1(P2,Middle,Right).
my_and_to_list1(true,Right,Right) :- !.
my_and_to_list1(P,[P|Right],Right).
