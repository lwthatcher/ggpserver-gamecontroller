...
public abstract class AbstractGameControllerRunner<
	...
	protected Game<TermType, ReasonerStateInfoType> createGame(File gameFile) throws IOException{
		String sightFileName = getSightFile();
		File sightFile = null;
		if (sightFile != null) {
			sightFile = new File(sightFileName);
		}
		return reasonerFactoryInterface.createGameFromFile(gameFile, gdlVersion, getStyleSheet(), sightFile);
	}

	protected Game<TermType, ReasonerStateInfoType> createGame(String gameDescription, String name) throws IOException{
		return reasonerFactoryInterface.createGame(gameDescription, name, gdlVersion, getStyleSheet(), getSightFile());
	}
